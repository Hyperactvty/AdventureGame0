...
// {
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        public bool GetFireInputHeld()
        {
            if (CanProcessInput())
            {
                bool isGamepad = Input.GetAxis(GameConstants.k_ButtonNameAttack) != 0f;
                if (isGamepad)
                {
                    return Input.GetAxis(GameConstants.k_ButtonNameAttack) >= TriggerAxisThreshold;
                }
                else
                {
                  if(Input.GetButton(GameConstants.k_ButtonNameAttack)) Debug.Log("Should be attacking...");
                    return Input.GetButton(GameConstants.k_ButtonNameAttack);
                }
            }
        ...
        public bool GetSkill1InputHeld()
        {
            if (CanProcessInput())
            {
                bool isGamepad = Input.GetAxis(GameConstants.k_ButtonNameGamepadSkill1) != 0f;
                bool i = isGamepad
                    ? (Input.GetAxis(GameConstants.k_ButtonNameGamepadSkill1) > 0f)
                    : Input.GetButton(GameConstants.k_ButtonNameSkill1);
                return i;
            }
