...
// {
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        public float AttackPerSecond = 0.5f;
        ...
        [Tooltip("Should the weapon use ammo?")]
        public bool WeaponUsesAmmo = false;
        ...
        void SyncWithPlayerStats()
        {
            psb = Owner.GetComponent<PlayerStatsBase>().playerStats;
            Attack_Bonus = psb.Attack;
            Attack_Speed = psb.Attack_Speed;
            Crit_Hit = psb.Crit_Hit;
            Crit_Damage = psb.Crit_Damage;

            Debug.Log($"Attacks Per Second : (( {AttackPerSecond} + ({Attack_Speed/50f})) * ({3f/10f}))\t = ({(( AttackPerSecond + (Attack_Speed/50f)) * /* alpha */(3f/10f))})");
        }
        ...
        bool TryShoot()
        {
            ...
            if(!WeaponUsesAmmo && m_LastTimeShot + (
              /* (( 1.4 + (0/50)) * (3/10)) */
              // (( AttackPerSecond + (Attack_Speed/50f)) * /* alpha */(3f/10f))
              _calculatedAPS
            ) < Time.time) {
              HandleShoot();
              return true;
